...
experts) of over 1500 different red wines. The goal of is this analysis is to 
illuminate what chemical and physical properties make wine taste good.
...
The median values of sulphates increase slightly as quality increases. 
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(redwines, aes(sulphates, citric.acid)) +
  geom_point(aes(color = quality)) +
  scale_color_brewer(type='qual')
```
...
```{r}
ggplot(redwines, aes(quality, fill = quality)) +
...
Quality is rated on a scale of 0-10, however the only scores that were assigned
were from 3-8. Most wines are mid quality with few low and high quality wines.
Any clustering of quality=7 and quality=8 wines is important in analysis.
...
to quality. Quality has a positive correlation with alcohol and a negative
correaltion with volatile acidity. 

Most high quality wines exceed 11% alcohol by volume and have below .5 g/dm^3
of volatile acids (acetic acid).
...
ggplot(aes(sulphates, citric.acid), data = redwines) +
  geom_jitter(aes(color = quality)) +
  scale_color_brewer(type='qual') +
  ggtitle('Citric Acid by Volatile Acidity with Quality') +
  theme(plot.title = element_text(size = 15, hjust = .5))
```

#### Description of Plot 2

Many of the higher tier alcohols (quality=7 and quality=8) have citric acid 
citric acid values above .25 and a range of sulphates [.5, 1.0]. 
...
Generally wines that are highly rated typically follow 4 main guidelines:

1) Percent Alcohol above 11%
2) Volatile Acidity (acetic acid) below .5 g/dm^3
3) Sulphates are between .5 g/dm^3 and 1.0 g/dm^3
4) Citric Adid above .25 g/dm^3


There are exceptions for high quality wines that do not all these guidelines, 
but most good wines have at least a few of these qualities.
